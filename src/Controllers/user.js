...
let UserController = class UserController {
     ...
     */
    handleGoogleLogin({ email, name, picture }) {
        return __awaiter(this, void 0, void 0, function* () {
            const user = yield UserService.handleGoogleSignIn({ email, name, picture });
            if (user) {
                const token = yield new Promise((resolve, reject) => {
                    jwt.sign(user, config.get("jwtSecret"), { expiresIn: 3600 }, (err, token) => {
                        if (err) {
                            reject(err);
                        }
                        else {
...
__decorate([
    (0, tsoa_1.Post)("/handleGoogleLogin"),
    __param(0, (0, tsoa_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], UserController.prototype, "handleGoogleLogin", null);
